...
#ifdef TARGET_PS3
    static int ps3_ucode_logs = 0;
    if (ps3_ucode_logs < 8) {
        this->Printf0("[PS3/EMU64] ucode switch type=%d ptr=%08x\n", this->ucode_type, (u32)this->gfx.words.w1);
        ps3_ucode_logs++;
        if (ps3_ucode_logs == 8) {
            this->Printf0("[PS3/EMU64] suppressing ucode switch logs\n");
        }
    }
#endif
    ...
    switch (moveword->index) {
        case G_MW_SEGMENT: {
            ...
            if (segment >= EMU64_NUM_SEGMENTS ||
                ...
                sprintf(s3, "gfxp=%s", this->segchk((u32)(uintptr_t)this->gfx_p));
...
u32 emu64::emu64_taskstart_r(Gfx* dl_p) {
    ...
    while (!this->end_dl && !FrameCansel) {
...
#ifdef TARGET_PS3
        if (this->ucode_type == UCODE_TYPE_SPRITE_TEXT) {
            switch (this->gfx_cmd) {
                ...
                case G_OBJ_RENDERMODE: {
                    if (this->gfx_cmd == G_RDPHALF_0) {
                        this->rdpHalf_1 = (u32)this->gfx.words.w1;
                    }

                    static int ps3_s2dex_skip_logs = 0;
                    if (ps3_s2dex_skip_logs < 8) {
                        this->Printf0("[PS3/EMU64] S2DEX cmd skipped op=%02x w0=%08x w1=%08x ucode=%d\n",
                                      this->gfx_cmd, this->gfx.words.w0, (u32)this->gfx.words.w1, this->ucode_type);
                        ps3_s2dex_skip_logs++;
                        if (ps3_s2dex_skip_logs == 8) {
                            this->Printf0("[PS3/EMU64] suppressing S2DEX skip logs\n");
                        }
                    }
        ...
        if (!command_handled && cmd_index < NUM_COMMANDS) {
            ...
            static int unexpected_cmd_count = 0;
            this->num_unknown_cmds++;
            if (this->gfx.words.w0 == ((u32)SYSDYNAMIC_END_MAGIC << 16)) {
                static int end_magic_cmd_count = 0;
                if (end_magic_cmd_count < 8) {
                    this->Printf0(
                        "[PS3/EMU64] dynamic display list reached end marker ptr=%08x cmd=%u ucode=%d level=%d\n",
                        (u32)(uintptr_t)this->gfx_p, this->cmds_processed, this->ucode_type, this->DL_stack_level);
                    end_magic_cmd_count++;
                    if (end_magic_cmd_count == 8) {
                        this->Printf0("[PS3/EMU64] suppressing dynamic end-marker logs\n");
                    }
                }
            } else {
                if (unexpected_cmd_count < 16) {
                    this->Printf0(
                        "[PS3/EMU64] unexpected cmd stop ptr=%08x cmd=%u op=%02x w0=%08x w1=%08x ucode=%d level=%d\n",
                        (u32)(uintptr_t)this->gfx_p, this->cmds_processed, this->gfx_cmd, this->gfx.words.w0,
                        (u32)this->gfx.words.w1, this->ucode_type, this->DL_stack_level);
                    unexpected_cmd_count++;
                    if (unexpected_cmd_count == 16) {
                        this->Printf0("[PS3/EMU64] suppressing unexpected command logs\n");
                    }
                }
            }
            this->end_dl = TRUE;
