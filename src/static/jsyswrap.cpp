...
static void* JW_EnsureDisplayManager(const char* caller) {
...
#ifdef TARGET_PS3
    OSReport("[PS3/JW] %s: display manager is null; recreating\n", caller);
#endif

    void* heap = JC_JFWSystem_getSystemHeap();
    if (heap == nullptr) {
#ifdef TARGET_PS3
        OSReport("[PS3/JW] %s: no system heap for display manager\n", caller);
#endif
        return nullptr;
    }

    displayManager = JC_JFWDisplay_createManager_0(&GXNtsc480IntDf, heap, 2, 1);
#ifdef TARGET_PS3
    OSReport("[PS3/JW] %s: display manager=%p heap=%p\n", caller, displayManager, heap);
#endif
    ...
    if (jc_fader != nullptr) {
        JC_JFWDisplay_setFader(displayManager, jc_fader);
    }

    JC_JFWDisplay_setClearColor(displayManager, TCOLOR_BLACK);
    JC_JFWDisplay_setGamma(displayManager, 0);
...
extern void JW_BeginFrame() {
    ...
    FrameDrawing = true;
    void* displayManager = JW_EnsureDisplayManager("JW_BeginFrame");
    if (displayManager == nullptr || jc_gport == nullptr) {
        FrameDrawing = false;
        return;
    }
#ifdef TARGET_PS3
    /* The PS3 GX shim already completes the render work in JW_EndFrame().
     * Calling JFWDisplay::endFrame() here corrupts the caller frame on the
     * second pump, so leave the frame rollover to the explicit end call. */
#else
...
extern void JW_EndFrame() {
    ...
    return;
#endif
    void* mgr = JW_EnsureDisplayManager("JW_EndFrame");
    if (mgr == nullptr) {
        FrameDrawing = false;
        return;
    }
...
extern int JW_setClearColor(u8 r, u8 g, u8 b) {
    ...
    if (clear_color.r != r || clear_color.g != g || clear_color.b != b) {
        void* manager = JW_EnsureDisplayManager("JW_setClearColor");
        if (manager == nullptr) {
            return FALSE;
        }
...
extern void JW_Init() {
...
#ifdef TARGET_PS3
    OSReport("[PS3/JW] JW_Init arena=%p-%p sysHeapSize=%08x\n", arena_lo, arena_hi, SystemHeapSize);
#endif
    ...
    JC_JFWSystem_init();
    void* system_heap = JC_JFWSystem_getSystemHeap();
#ifdef TARGET_PS3
    OSReport("[PS3/JW] JW_Init systemHeap=%p\n", system_heap);
#endif
    JC_JKRExpHeap_changeGroupID(system_heap, 1);
    ...
    void* display_manager = JC_JFWDisplay_createManager_0(&GXNtsc480IntDf, system_heap, 2, 1);
#ifdef TARGET_PS3
    OSReport("[PS3/JW] JW_Init displayManager=%p current=%p\n", display_manager, JC_JFWDisplay_getManager());
#endif
    ...
    JW_SetVideoPan(20, 6, 598, 464);
    JW_SetLogoMode(1);
    JC_JKRExpHeap_changeGroupID(system_heap, 2);
}
...
extern void JW_Cleanup() {
    ...
    if (JC_JFWDisplay_getManager() != nullptr) {
