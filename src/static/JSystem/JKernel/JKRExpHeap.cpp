...
JKRExpHeap* JKRExpHeap::createRoot(int maxHeaps, bool errorFlag) {
    ...
    void* memory = nullptr;
    u32 memorySize = 0;
    if (!initArena((char**)&memory, &memorySize, maxHeaps) || !memory) {
        return nullptr;
    }

    u32 heapHeaderSize = ALIGN_NEXT(sizeof(JKRExpHeap), 0x10);
    if (memorySize <= heapHeaderSize + sizeof(CMemBlock)) {
        return nullptr;
    }

    u8* start = (u8*)memory + heapHeaderSize;
    u32 alignedSize = memorySize - heapHeaderSize;
    JKRExpHeap* heap = new (memory) JKRExpHeap(start, alignedSize, nullptr, errorFlag);
    if (!heap) {
        return nullptr;
    }
...
void* JKRExpHeap::allocFromHead(u32 size, int align) {
    ...
    for (CMemBlock* block = mHead; block; block = block->mNext) {
        uintptr_t content = (uintptr_t)block->getContent();
        u32 offset = (u32)(ALIGN_PREV((uintptr_t)align - 1 + content, (uintptr_t)align) - content);
...
void* JKRExpHeap::allocFromTail(u32 size, int align) {
    ...
    for (CMemBlock* block = mTail; block; block = block->mPrev) {
        uintptr_t content = (uintptr_t)block->getContent();
        start = ALIGN_PREV(content + block->mAllocatedSpace - size, (uintptr_t)align);
        usedSize = (u32)(content + block->mAllocatedSpace - start);
