...
void JFWSystem::firstInit() {
    ...
    if (!systemHeap) {
        systemHeap = JKRHeap::getSystemHeap();
    }
    if (!systemHeap) {
        systemHeap = rootHeap;
    }
...
void JFWSystem::init() {
    ...
    void* mem = systemHeap ? systemHeap->alloc(CSetUpParam::exConsoleBufferSize, 4) : nullptr;
    if (!mem) {
        mem = JKRHeap::alloc(CSetUpParam::exConsoleBufferSize, 4, nullptr);
    }
    if (mem) {
