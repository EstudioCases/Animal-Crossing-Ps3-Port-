...
extern u32 Neos_Update(s16* dst) {
    ...
    /* Diagnostic: check NEOS output amplitude every 60 frames */
    {
...
#ifdef TARGET_PS3
        static u32 neos_diag_logs = 0;
        if ((neos_diag_ctr++ % 300) == 0 && neos_diag_logs < 8) {
#else
        if ((neos_diag_ctr++ % 60) == 0) {
            ...
            neos_diag_logs++;
            if (neos_diag_logs == 8) {
                printf("[NEOS_OUT] suppressing PS3 audio diagnostic logs\n");
            }
#endif
        }
    }
...
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] pc_neos_init_sync begin\n");
#endif
...
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] NEOS rom_top=%x preloaded=%x file_top=%x\n", neos_rom_top, neos_rom_preloaded, neos_file_top);
#endif
...
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] NEOS DVD process begin\n");
#endif
    pc_dvd_process_all_tasks();
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] NEOS DVD process done\n");
#endif
...
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] pc_neos_init_sync done\n");
#endif
}
