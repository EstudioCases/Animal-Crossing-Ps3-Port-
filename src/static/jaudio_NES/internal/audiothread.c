...
extern void StartAudioThread(void* pHeap, s32 heapSize, u32 aramSize, u32 flags) {
...
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] StartAudioThread begin flags=%x heapSize=%x aramSize=%x\n", flags, heapSize, aramSize);
#endif
...
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] ARAM initialized neos=%x\n", neos_flag);
#endif
    ...
    Jac_Init();
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] Jac_Init returned\n");
#endif
    Jac_InitSinTable();
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] Jac_InitSinTable returned\n");
#endif
    ResetPlayerCallback();
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] ResetPlayerCallback returned\n");
#endif
    DspbufProcess(DSPBUF_EVENT_INIT);
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] DspbufProcess INIT returned\n");
#endif
    CpubufProcess(DSPBUF_EVENT_INIT);
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] CpubufProcess INIT returned\n");
#endif
    /* Skip DspBoot() — no DSP microcode on PC */
    DSP_InitBuffer();
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] DSP_InitBuffer returned\n");
#endif
    AISetDSPSampleRate(JAC_AI_SETTING);
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] AISetDSPSampleRate returned\n");
#endif
    AIStartDMA();

#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] AI DMA started\n");
#endif
    ...
    if ((flags & AUDIO_THREAD_FLAG_DVD)) {
        jac_dvdproc_init();
#ifdef TARGET_PS3
        OSReport("[PS3/AUDIO] DVD proc initialized\n");
#endif
    }
    ...
    if (neos_flag) {
#ifdef TARGET_PS3
        OSReport("[PS3/AUDIO] NEOS init begin\n");
#endif
        pc_neos_init_sync();
#ifdef TARGET_PS3
        OSReport("[PS3/AUDIO] NEOS init done\n");
#endif
    }
...
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] StartAudioThread done\n");
#endif
}
