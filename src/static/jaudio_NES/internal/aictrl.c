...
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] Jac_Init begin DAC_SIZE=%u\n", DAC_SIZE);
#endif
    for (int i = 0; i < 3; i++) {
...
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] Jac_Init buffers allocated %p %p %p\n", dac[0], dac[1], dac[2]);
#endif
    AIInit(nullptr);
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] Jac_Init before initial DMA\n");
#endif
    AIInitDMA((u32)dac[2], DAC_SIZE * 2);
#ifdef TARGET_PS3
    OSReport("[PS3/AUDIO] Jac_Init done\n");
#endif
}
