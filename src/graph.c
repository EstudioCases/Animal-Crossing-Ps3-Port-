...
#ifdef TARGET_PS3
static int graph_require_gfx_room(const char* name, THA_GA* arena, int gfx_count) {
    int free_bytes = THA_GA_getFreeBytes(arena);
    int need_bytes = gfx_count * (int)sizeof(Gfx);

    if (free_bytes < need_bytes) {
        static int log_count = 0;
        if (log_count < 16) {
            printf("[PS3/GRAPH] %s display list full before finish: free=%d need=%d\n",
                   name, free_bytes, need_bytes);
            log_count++;
            if (log_count == 16) {
                printf("[PS3/GRAPH] suppressing display-list capacity logs\n");
            }
        }
        return FALSE;
    }
...
static int graph_check_arena(const char* name, THA_GA* arena) {
    if (THA_GA_isCrash(arena)) {
        static int log_count = 0;
        if (log_count < 16) {
            printf("[PS3/GRAPH] %s display list overflow: free=%d\n",
                   name, THA_GA_getFreeBytes(arena));
            log_count++;
            if (log_count == 16) {
                printf("[PS3/GRAPH] suppressing display-list overflow logs\n");
            }
        }
        return TRUE;
    }
...
static int graph_draw_finish(GRAPH* this) {
    int err;
#ifdef TARGET_PS3
    if (!graph_require_gfx_room("work", &this->work_thaga, 1) ||
        !graph_require_gfx_room("bg_opaque", &this->bg_opaque_thaga, 1) ||
        !graph_require_gfx_room("shadow", &this->shadow_thaga, 1) ||
        !graph_require_gfx_room("bg_translucent", &this->bg_translucent_thaga, 1) ||
        !graph_require_gfx_room("polygon_opaque", &this->polygon_opaque_thaga, 1) ||
        !graph_require_gfx_room("polygon_translucent", &this->polygon_translucent_thaga, 1) ||
        !graph_require_gfx_room("light", &this->light_thaga, 1) ||
        !graph_require_gfx_room("font", &this->font_thaga, 1) ||
        !graph_require_gfx_room("overlay", &this->overlay_thaga, 3)) {
        return TRUE;
    }
#endif
...
#ifdef TARGET_PS3
        err = TRUE;
        printf("[PS3/GRAPH] sys_dynamic start marker was overwritten\n");
#else
#if VERSION == VER_GAFU01_00
    ...
    if (!SYSDYNAMIC_CHECK_END()) {
        err = TRUE;
#ifdef TARGET_PS3
        printf("[PS3/GRAPH] sys_dynamic end marker was overwritten\n");
#else
#if VERSION == VER_GAFU01_00
...
#ifdef TARGET_PS3
    if (graph_check_arena("work", &this->work_thaga)) {
        err = TRUE;
    }
#endif
