/* ps3_card.c - memory card API backed by files in the package save dir.
 * Channel 0 (Slot A) maps to card_a, channel 1 (Slot B) maps to card_b. */
...
typedef struct {
    CARDFileInfo_PS3* owner;  /* NULL = slot free */
    FILE*            fp;
    char             filename[64];
} CARDOpenSlot;
...
static CARDOpenSlot* card_slot_alloc(CARDFileInfo_PS3* fi) {
    int i;
...
static CARDOpenSlot* card_slot_find(CARDFileInfo_PS3* fi) {
    int i;
    for (i = 0; i < CARD_MAX_OPEN; i++) {
...
static void card_slot_free(CARDFileInfo_PS3* fi) {
    int i;
    for (i = 0; i < CARD_MAX_OPEN; i++) {
...
static void ensure_dirs(void) {
    mkdir_if_needed(PS3_SAVE_DIR);
    mkdir_if_needed(PS3_SAVE_DIR "/card_a");
    mkdir_if_needed(PS3_SAVE_DIR "/card_b");
}
...
s32 CARDOpen(s32 chan, const char* fileName, CARDFileInfo_PS3* fileInfo) {
    char path[512];
    CARDOpenSlot* slot;
...
s32 CARDClose(CARDFileInfo_PS3* fileInfo) {
...
s32 CARDCreate(s32 chan, const char* fileName, u32 size, CARDFileInfo_PS3* fileInfo) {
    char path[512];
    CARDOpenSlot* slot;
    if (!card_filename_safe(fileName)) return CARD_RESULT_NAMETOOLONG;
    snprintf(path, sizeof(path), "%s/%s", get_card_dir(chan), fileName);
...
s32 CARDCreateAsync(s32 chan, const char* fileName, u32 size, void* fileInfo, void* callback) {
    s32 result = CARDCreate(chan, fileName, size, (CARDFileInfo_PS3*)fileInfo);
    if (callback) ((void (*)(s32, s32))callback)(chan, result);
    return result;
}

s32 CARDRead(CARDFileInfo_PS3* fileInfo, void* buf, s32 length, s32 offset) {
    CARDOpenSlot* slot = card_slot_find(fileInfo);
...
s32 CARDReadAsync(void* fileInfo, void* buf, s32 length, s32 offset, void* callback) {
    s32 result = CARDRead((CARDFileInfo_PS3*)fileInfo, buf, length, offset);
    if (callback) ((void (*)(s32, s32))callback)(0, result);
    return result;
}

s32 CARDWrite(CARDFileInfo_PS3* fileInfo, const void* buf, s32 length, s32 offset) {
    CARDOpenSlot* slot = card_slot_find(fileInfo);
    if (!slot || !slot->fp) return CARD_RESULT_NOFILE;
    fseek(slot->fp, offset, SEEK_SET);
...
s32 CARDWriteAsync(void* fileInfo, const void* buf, s32 length, s32 offset, void* callback) {
    s32 result = CARDWrite((CARDFileInfo_PS3*)fileInfo, buf, length, offset);
    if (callback) ((void (*)(s32, s32))callback)(0, result);
    return result;
}
...
int ps3_card_scan_for_gci(s32 chan, char* out_path, int out_size) {
