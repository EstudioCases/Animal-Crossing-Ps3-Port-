/* ps3_os.c - Dolphin OS replacement: arena, timers, threads, message queues */
...
/* --- Time/clock --- */
u32 ps3_OSBusClock  = GC_BUS_CLOCK;
u32 ps3_OSCoreClock = GC_CORE_CLOCK;
...
static s64 usec_to_gc_ticks(u64 usec) {
    return (s64)(usec * (GC_TIMER_CLOCK / 1000000ULL)) +
           (s64)((usec * (GC_TIMER_CLOCK % 1000000ULL)) / 1000000ULL);
}

s64 osGetTime(void) {
    u64 now = ps3_platform_time_us();
    s64 elapsed = usec_to_gc_ticks(now - time_base_start);
...
void OSInit(void) {
    if (!arena_memory) {
        arena_alloc = (u8*)malloc(PS3_MAIN_MEMORY_SIZE + 31);
        if (arena_alloc) {
            arena_memory = (u8*)(((uintptr_t)arena_alloc + 31u) & ~(uintptr_t)31u);
        }
        if (!arena_memory) {
            fprintf(stderr, "Failed to allocate main memory arena\n");
            exit(1);
        }
        memset(arena_memory, 0, PS3_MAIN_MEMORY_SIZE);

        ps3_arena_base = arena_memory;
        ps3_arena_end  = arena_memory + PS3_MAIN_MEMORY_SIZE;

        /* GC system info at phys addr 0; offset 0x28 = mem size for JKRHeap */
        *(u32*)(arena_memory + 0x28) = PS3_MAIN_MEMORY_SIZE;
    ...
    /* compute ticks from GC epoch (Jan 1, 2000) to now, with timezone */
    {
        ...
        if (g_ps3_time_override >= 0) {
            /* --time H[:M[:S]] override */
            struct tm* lt = localtime(&unix_now);
            if (lt) {
                unix_now += (g_ps3_time_override - lt->tm_hour) * 3600;
                if (g_ps3_min_override >= 0)
                    unix_now += (g_ps3_min_override - lt->tm_min) * 60;
                if (g_ps3_sec_override >= 0)
                    unix_now += (g_ps3_sec_override - lt->tm_sec);
            }
        }
...
void msleep(int ms) { ps3_platform_sleep_ms((unsigned int)ms); }
...
u32 OSGetPhysicalMemSize(void) { return PS3_MAIN_MEMORY_SIZE; }
