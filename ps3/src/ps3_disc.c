 ...
 * Used by ps3_assets.c for DOL+REL extraction and ps3_dvd.c for runtime file reads. */
#ifdef TARGET_PS3
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void build_fst_table(DiscFile* df) {
    ...
    if (g_ps3_verbose) {
        printf("[PS3] FST: %d files indexed\n", g_fst_file_count);
        for (int fi = 0; fi < g_fst_file_count; fi++)
            printf("[PS3] FST[%d]: %s (%u bytes @ 0x%X)\n", fi,
...
int ps3_disc_init(void) {
    ...
    if (!gcm_verify(&g_disc)) {
        if (g_ps3_verbose) printf("[PS3] %s: not a valid GC disc image\n", path);
        disc_close(&g_disc);
        return 0;
    }

    {
        ...
        id[6] = '\0';
        if (g_ps3_verbose) printf("[PS3] Disc image: %s (%s, %s)\n",
...
u8* ps3_disc_extract_dol(void) {
    ...
    if (g_ps3_verbose)
        printf("[PS3] DOL: %u bytes (offset 0x%X)\n", g_dol_size, g_dol_offset);
    return buf;
}

u8* ps3_disc_extract_rel(void) {
    u32 off, sz;
    u8* raw;

    if (!ps3_disc_find_file("foresta.rel.szs", &off, &sz)) {
        if (g_ps3_verbose) printf("[PS3] foresta.rel.szs not found in disc FST\n");
        return NULL;
    }
    ...
    if (sz >= 16 && memcmp(raw, "Yaz0", 4) == 0) {
        ...
        free(raw);
        if (!dec) {
            if (g_ps3_verbose) printf("[PS3] Yaz0 decompression failed\n");
            return NULL;
        }
        if (g_ps3_verbose)
            printf("[PS3] REL: %u bytes (Yaz0: %u -> %u)\n", dec_sz, sz, dec_sz);
        return dec;
    }

    if (g_ps3_verbose) printf("[PS3] REL: %u bytes (raw)\n", sz);
    return raw;
}
